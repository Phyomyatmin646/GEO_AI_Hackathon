...
export default function FAQPage() {
  ...
  useEffect(() => {
    async function fetchFaqs() {
      setLoading(true);
      try {
        const res = await fetch(`/api/v1/faq?search=${encodeURIComponent(search)}`);
      ...
      <header className="topbar bg-white shadow-sm flex items-center justify-between p-4 border-b">
        ...
        <div className="topbar-status flex items-center gap-4">
          <Link href="/" className="text-sm text-blue-600 hover:underline">
            {lang === "en" ? "Back to Dashboard" : "Dashboard သို့ ပြန်သွားရန်"}
          </Link>
          <button 
            ...
            faqs.map((faq) => (
              <div key={faq.faq_id} className="bg-white p-6 rounded-lg shadow-sm border">
                ...
                <h3 className="text-xl font-semibold mb-3">
                  {lang === "en" ? (faq.question_en !== "Pending English Translation" ? faq.question_en : faq.question_mm) : faq.question_mm}
                </h3>
                <div className="text-gray-700 leading-relaxed whitespace-pre-wrap">
                  {lang === "en" ? (faq.answer_en !== "Pending English Translation" ? faq.answer_en : faq.answer_mm) : faq.answer_mm}
                </div>
                
                <div className="mt-4 pt-4 border-t text-xs text-gray-400 flex justify-between">
                  <span>Source: {faq.source_title}</span>
                  <span>Reviewed: {new Date(faq.last_reviewed_at).toLocaleDateString()}</span>
                </div>
              </div>
            ))
          )}
